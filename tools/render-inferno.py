...
import ctypes as c,json,numpy as np
...
data=json.loads(Path('.dream-loop/inferno-draws.json').read_text());programs=[]
...
for unit,name in [(5,'inferno-flame-paint.png'),(6,'inferno-atlas.png'),(7,'inferno-inbetweens.png'),(8,'inferno-inbetweens48.png')]:
...
root=Path('public/review');kind=data.get('kind','inferno')
if len(images)>6:images[0].save(root/(kind+'-motion.gif'),save_all=True,append_images=images[1:],duration=20,loop=0)
else:
 sheet=Image.new('RGB',(1536,1076),'#eeeadc')
 for i,im in enumerate(images):sheet.paste(im,((i%3)*512,(i//3)*538));ImageDraw.Draw(sheet).text(((i%3)*512+8,(i//3)*538+514),f"{data['frames'][i]['t']:.2f}s after impact",fill='black')
 sheet.save(root/(kind+'-keyframes.png'))
print('Actual GLES shaders:',len(programs),'rendered frames:',len(images),'GL errors:0')
