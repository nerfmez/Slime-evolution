...
test('atlas contains front/back for 9 EXP animals, panda roll, and three item animals',()=>{assert.equal(EXP_FRAMES.length,3);assert.ok(EXP_FRAMES.every(set=>set.length===3&&set.every(pair=>pair.length===2)));assert.ok(SPRITES[PANDA_ROLL]);for(const kind of ['heal','magnet','nova']){assert.equal(ITEM_FRAMES[kind].length,2);for(const f of ITEM_FRAMES[kind])assert.ok(SPRITES[f]);}const webp=readFileSync(new URL('../critters/atlas.webp',import.meta.url));assert.equal(webp.subarray(0,4).toString(),'RIFF');assert.equal(webp.subarray(8,12).toString(),'WEBP');assert.ok(ATLAS_URL.endsWith('/critters/atlas.webp'));});
