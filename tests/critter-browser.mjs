...
try{
 ...
 await page.route('**/main-critter-v3.js',route=>route.fulfill({contentType:'text/javascript',body:readFileSync('published/2026-09-14/assets/main-critter-v3.js','utf8').replaceAll('requestAnimationFrame(Zr)','(window.__critterFreeze||requestAnimationFrame(Zr))')+'\nwindow.__critterTest={combat:Y,world:J,state:W,renderer:()=>Hr,vp:()=>X,frame:Zr};'}));
 ...
 await page.evaluate(()=>{
  ...
  c.souls=Array.from({length:3},(_,i)=>({x:s.player[0]+(i-1)*1.1,z:s.player[2]-2.2,id:i+1,value:[2,10,56][i],age:1,critter:{variant:i,seed:(i+.5)/3,homeX:0,homeZ:0,facing:1,phase:.5,moving:0,eat:0,finished:false}}));
 ...
 const catalog=await browser.newPage({viewport:{width:940,height:1940},deviceScaleFactor:1});
 catalog.on('pageerror',e=>errors.push(String(e)));
 await catalog.goto('http://127.0.0.1:8765/exp-animals.html',{waitUntil:'networkidle'});
 await catalog.waitForTimeout(600);await catalog.locator('#motion').click();
 assert.equal(await catalog.locator('.tier').count(),9);assert.equal(await catalog.locator('.cell').count(),27);
 assert.equal(await catalog.locator('#error').isVisible(),false);
 assert.equal(await catalog.locator('#catalog').evaluate(c=>c.getContext('webgl2').getError()),0);
 await catalog.screenshot({path:'/tmp/critter-review/exp-catalog.png',fullPage:true});
 await catalog.setViewportSize({width:390,height:844});await catalog.waitForTimeout(200);
 assert.equal(await catalog.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth),true);
 await catalog.screenshot({path:'/tmp/critter-review/exp-catalog-mobile.png',fullPage:true});await catalog.close();
 const result=await page.evaluate(()=>{
  ...
  setup();c.souls=[{x:p[0]+8,z:p[2],age:1,value:2,id:199}];step(1);const actor=c.souls[0].critter;actor.fleeArmed=false;actor.fleeLeft=0;const home=[actor.homeX,actor.homeZ],form=actor.expForm;
  w.defeated=[{type:'crystal',x:p[0]+8,z:p[2],xp:54,hp:0}];step(1);const merged={value:c.souls[0].value,tier:actor.expTier,sameActor:actor===c.souls[0].critter,sameForm:form===actor.expForm,sameHome:home[0]===actor.homeX&&home[1]===actor.homeZ};
  setup();const many=Array.from({length:1000},(_,i)=>({x:p[0]+(i%25)*.08,z:p[2]+Math.floor(i/25)*.05,value:[2,5,6,10,12,15,18,30,56][i%9],id:i+1,age:1}));
  const batch=renderer().draw(many,vp(),1,p,()=>true);const glError=document.getElementById('world').getContext('webgl2').getError();
  setup();s.paused=false;window.__critterFreeze=false;requestAnimationFrame(window.__critterTest.frame);return {noModXP,noModLaterXP,escapeDistance,firstModXP,heal,magnetXP,repeatedXP,novaHP,merged,batch,glError};
 });
 writeFileSync('/tmp/critter-review/integration.json',JSON.stringify(result,null,2));
 assert.deepEqual(result.merged,{value:56,tier:8,sameActor:true,sameForm:true,sameHome:true});assert.equal(result.noModXP,0);assert.equal(result.noModLaterXP,0);assert.ok(result.escapeDistance>.9);assert.ok(result.escapeDistance<1.4);assert.equal(result.firstModXP,2);assert.equal(result.heal,62);assert.equal(result.magnetXP,7);assert.equal(result.repeatedXP,7);assert.equal(result.novaHP[0],0);assert.equal(result.novaHP[1],30);assert.equal(result.batch.calls,1);assert.equal(result.batch.count,1000);assert.equal(result.glError,0);
