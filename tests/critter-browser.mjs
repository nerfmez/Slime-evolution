...
try{
 ...
 await page.route('**/main-critter-v1.js',route=>route.fulfill({contentType:'text/javascript',body:readFileSync('published/2026-09-14/assets/main-critter-v1.js','utf8').replaceAll('requestAnimationFrame(Zr)','(window.__critterFreeze||requestAnimationFrame(Zr))')+'\nwindow.__critterTest={combat:Y,world:J,state:W,renderer:()=>Hr,vp:()=>X,frame:Zr};'}));
 ...
 const result=await page.evaluate(()=>{
  ...
  setup();s.paused=false;window.__critterFreeze=false;requestAnimationFrame(window.__critterTest.frame);return {heal,magnetXP,repeatedXP,novaHP,batch,glError};
 });
