...
try{
 ...
 const cases=await page.evaluate(async()=>{
  ...
  const quiet=[];for(const time of [45,105,155,215,275,340,400,455,520,575]){w.reset('auto',time);w.update(.05,p,100);quiet.push({time,count:w.enemies.length});}
  ...
  w.reset('auto',420);w.update(.05,p,100);const elite={type:w.enemies[0]?.type,elite:w.enemies[0]?.elite,hp:w.enemies[0]?.maxHP,damage:w.enemies[0]?.cozyStats.damage,xp:w.enemies[0]?.xp};
  ...
  const c=q.combat;c.reset();c.cards=[];c.opening=false;c.cooldown=1e6;c.update(.01,w,p);const dropped=c.souls.reduce((sum,s)=>sum+s.value,0);
  ...
  return {intro,first,quiet,unlocks,capped,quietKeepsSurvivors,firstPanda,noStack,bossDeferred,bossAfterClear,secondPanda,elite,beforeBoss,atBoss,noAdds,victory,reward:{expectedXP,dropped,notDuplicated,collected},training};
 });report.cases=cases;
 ...
 assert.deepEqual(cases.secondPanda,{hp:640,xp:140});assert.deepEqual(cases.elite,{type:'water',elite:true,hp:420,damage:14,xp:100});
 ...
 assert.equal(cases.reward.dropped,cases.reward.expectedXP);assert.ok(cases.reward.notDuplicated);assert.equal(cases.reward.collected,cases.reward.expectedXP);assert.deepEqual(cases.training,{hp:480,xp:45});
