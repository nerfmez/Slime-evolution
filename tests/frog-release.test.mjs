...
test('authoritative release contains natural directional Moss Frog renderer, not old normal Thorn sprite request',()=>{
 const js=readFileSync(bundlePath,'utf8');
 assert.match(js,/name:`Moss Frog`/);
 assert.match(js,/frog-moveset\.png\?v=20260916-hop1/);
 assert.match(js,/globalThis\.__slimeFrogFrame=Fg/);
 assert.match(js,/globalThis\.__slimeFrogFacing=Fh/);
 assert.match(js,/globalThis\.__slimeFrogHopLift=Fl/);
 assert.match(js,/uniform float flipX/);
 assert.match(js,/spriteOffsetX',c\.o\*l/);
 assert.match(js,/spriteScale',\(t\.scale\|\|1\)\*\.714/);
 assert.match(js,/origin',\[t\.x,\.02\+u,t\.z\]/);
 assert.match(js,/frogHopPhase:0/);
 assert.match(js,/frogHopActive:!1/);
 assert.match(js,/frogMoveScale/);
 assert.match(js,/i\.speed\*e\*frogMoveScale/);
