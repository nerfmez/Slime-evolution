...
import {insideEliteFrogCone,eliteFrogAttack,ELITE_FROG_WINDUP,eliteFrogTonguePoints,tongueTouchesPlayer} from '../species/elite-frog-combat.js';
...
test('damage follows the visible tongue instead of hitting the entire cone at once',()=>{
 ...
 assert.ok(p.at(-1).y>Math.min(...p.map(q=>q.y))+1.4); // Raised return hook, matching the video.
