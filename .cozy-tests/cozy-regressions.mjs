...
export function adaptBrowserSource(name,source){
 const edits=[];
 const click="await page.locator('.skill-card').first().click({force:true});await page.evaluate(()=>{const w=globalThis.__slimeGameQA?.world;if(w?.mode==='auto')w.time=Math.max(w.time,15)});";
