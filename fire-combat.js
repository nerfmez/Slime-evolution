...
export class FireCombat{
 ...
 hit(e,damage,x,z,push=0){if(e.hp<=0)return;let applied=damage;if(e.isBoss)applied=Math.max(1,Math.round(applied*.72));if(e.type==='petal'&&e.isElite&&e.flying)applied=Math.max(1,Math.round(applied*1.5));if(e.guardTime>0)applied=Math.max(1,Math.round(applied*.42));e.hp-=applied;e.hit=.12;if(this.numbers.length<48)this.numbers.push({x:e.x,z:e.z,value:applied,age:0});
  if(push){const d=Math.hypot(e.x-x,e.z-z)||1,resist=e.isBoss?.06:e.type==='petal'?(e.isElite?.12:.22):e.type==='panda'?(e.isElite?.18:.30):e.type==='crystal'?(e.isElite?.32:.55):e.type==='moss'?(e.isElite?.55:.82):e.isElite?.55:1,scale=resist*push*.10,nx=e.x+(e.x-x)/d*scale,nz=e.z+(e.z-z)/d*scale;if(enemyCanStand(nx,nz,e.radius)){e.x=nx;e.z=nz;}}
 }
 ...
 update(dt,world,player){
  ...
  for(const c of this.cyclones){c.age+=dt;c.life-=dt;c.tick-=dt;const target=this.target(world,[c.x,0,c.z]);if(target){const dx=target.x-c.x,dz=target.z-c.z,d=Math.hypot(dx,dz),step=Math.min(d,c.speed*dt);if(d){c.x+=dx/d*step;c.z+=dz/d*step;}}for(const e of world.enemies){const d=Math.hypot(e.x-c.x,e.z-c.z);if(e.hp<=0||d>c.r)continue;if(c.tick<=0){this.hit(e,Math.max(2,Math.round(c.s.damage*.10)),c.x,c.z);this.burn(e,c.s.burn,c.s.burnTime*.78);}if(d>.35){const pull=dt*(e.type==='petal'?.15:e.type==='panda'?.25:.5),x=e.x+(c.x-e.x)/d*pull,z=e.z+(c.z-e.z)/d*pull;if(enemyCanStand(x,z,e.radius)){e.x=x;e.z=z;}}}if(c.tick<=0)c.tick=.25;}this.cyclones=this.cyclones.filter(c=>c.life>0);
