...
const grassBend=createGrassBend(),labGrassBend=createGrassBend();let grassBendTexture,projectileMesh;
...
let enemyAssets={},thornSprite,waterCalfSprite,fireVFX,soulVFX,vegetationDepth;
...
function renderScene(){
 ...
 for(const e of visibleEnemies){
  ...
  uniform(gl,prog,'enemyLift',e.isBoss?.48:e.type==='moss'?.22:e.type==='water'?.25:e.flying?.08:.18);
  uniform(gl,prog,'enemyPalette',e.isBoss?4:e.type==='moss'?1:e.type==='water'?2:e.type==='petal'?3:0);
 ...
 if(!proofCamera){
  ...
  for(const e of visibleEnemies)if(e.skillWindup>0&&e.skillRadius>0){const targeted=['moss_pillar_line','water_shot','vine_lunge'].includes(e.skillKind),x=targeted?e.skillTargetX:e.x,z=targeted?e.skillTargetZ:e.z;uniform(gl,prog,'enemySkillColor',e.skillColor||[1,.78,.38]);draw(quad,33,model(x,.025,z,e.skillRadius,1,e.skillRadius),Math.min(1,.35+e.skillWindup));}
  ...
  uniform(gl,prog,'enemySkillColor',[.30,.72,1.00]);
...
function updateEncounterHUD(){
 ...
 else if(enemyWorld.mode==='elites')$('roster').textContent=['thorn','moss','petal','water'].map(k=>ENEMY_TYPES[k].eliteName).join(' · ');
