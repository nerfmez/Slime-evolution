...
// Compact paper tags sized from measured glyph bounds (Thai fonts differ per OS): star + Thai name, HP only once damaged, stacked instead of overlapping, faded over the player.
const ELITE_NEW='{const names={thorn:`กบมอส`,spark:`เม่นสายฟ้า`,turtle:`เต่าโล่พลัง`,water:`ช้างน้ำ`},tags=[],placed=[],[px,py]=proj(W.player[0],.45,W.player[2]);for(let e of J.enemies){if(!e.elite||e.hp<=0)continue;let a=({thorn:1.95,spark:1.08,turtle:1.25,water:1.38}[e.type]||1.15)*(e.scale||1),[o,s]=proj(e.x,a,e.z);tags.push({e,x:o,y:s-22})}tags.sort((a,b)=>b.y-a.y);r.font=`700 11px system-ui,sans-serif`,r.textAlign=`center`,r.textBaseline=`alphabetic`,r.lineWidth=1;for(let g of tags){let e=g.e,label=`★ `+(names[e.type]||pt(e).name),d=Math.max(0,Math.min(1,e.hp/Math.max(1,e.maxHP||e.hp))),hurt=d<.999,m=r.measureText(label),asc=m.actualBoundingBoxAscent||9,desc=m.actualBoundingBoxDescent||3,inkL=m.actualBoundingBoxLeft??m.width/2,inkR=m.actualBoundingBoxRight??m.width/2,th=asc+desc,w=Math.max(inkL+inkR+16,hurt?60:0),h=th+(hurt?14:9),x=g.x,y=g.y;for(let k=0;k<6;k++){let hit=placed.find(p=>Math.abs(p.x-x)<(p.w+w)/2&&Math.abs(p.y-y)<(p.h+h)/2);if(!hit)break;y=hit.y-(hit.h+h)/2-2}placed.push({x,y,w,h});let top=y-h/2;r.globalAlpha=Math.abs(px-x)<w/2+18&&Math.abs(py-y)<h/2+28?.38:1,r.beginPath(),r.roundRect?r.roundRect(x-w/2,top,w,h,9):r.rect(x-w/2,top,w,h),r.fillStyle=`rgba(247,240,220,.92)`,r.fill(),r.strokeStyle=`rgba(120,96,64,.55)`,r.stroke(),r.fillStyle=`#35483a`,r.fillText(label,x+(inkL-inkR)/2,top+4.5+asc);if(hurt){let bw=w-14,bx=x-bw/2,by=top+th+7.5;r.fillStyle=`#ddd3b6`,r.fillRect(bx,by,bw,4),r.fillStyle=d>.35?`#bd6658`:`#a8402f`,r.fillRect(bx,by,bw*d,4)}}r.globalAlpha=1}';

const BUNDLE_EDITS=[
  ['elite tags',ELITE_OLD,ELITE_NEW],
  ['boss banner','n?.miniBoss?`BAMBOO PANDA · MINI-BOSS`:`ANCIENT BLOOM COLOSSUS`','n?.miniBoss?`มินิบอส · แพนด้าไผ่`:`บอส · ยักษ์บุปผาโบราณ`'],
  ['boss status','J.bossSpawned?`ด่าน 1 · Ancient Bloom Colossus`:','J.bossSpawned?`ด่าน 1 · บอสยักษ์บุปผาโบราณ`:'],
...
const HTML_EDITS=[
  ['page title','<title>Slime — ฉากทดลองเว็บ</title>','<title>Slime — ทุ่งหญ้าสไลม์</title>'],
  ['header kicker','<small>SLIME · FIRE & SOULS</small>','<small>สไลม์ · ไฟและวิญญาณ</small>'],
  ['boss banner default','<strong>ANCIENT BLOOM COLOSSUS</strong>','<strong>บอส · ยักษ์บุปผาโบราณ</strong>'],
