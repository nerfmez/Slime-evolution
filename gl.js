...
export async function loadTexture(gl,url,mipmaps=true){
 const response=await fetch(url);if(!response.ok)throw Error('โหลดภาพไม่สำเร็จ '+url+' · HTTP '+response.status);
 let bitmap;
 try{bitmap=await createImageBitmap(await response.blob());}
 catch(error){throw Error('ถอดรหัสภาพไม่สำเร็จ '+url+' · '+(error?.message||error));}
