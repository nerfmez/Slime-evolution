...
const OPTIONAL_TEXTURES=['inferno-atlas.png','inferno-inbetweens.png','inferno-inbetweens48.png','sunfall-impact-hd.png','sunfall-flight-hd.png','sunfall-smoke-hd.png'];
export async function loadTexture(gl,url,mipmaps=true){const response=await fetch(url);if(!response.ok){if(OPTIONAL_TEXTURES.some(name=>url.endsWith(name))){console.warn('Optional texture unavailable:',url,response.status);return null;}throw Error('โหลด texture ไม่สำเร็จ: '+url+' ('+response.status+')');}const bitmap=await createImageBitmap(await response.blob());const texture=gl.createTexture();gl.bindTexture(gl.TEXTURE_2D,texture);gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,gl.RGBA,gl.UNSIGNED_BYTE,bitmap);if(mipmaps)gl.generateMipmap(gl.TEXTURE_2D);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,mipmaps?gl.LINEAR_MIPMAP_LINEAR:gl.LINEAR);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_S,gl.CLAMP_TO_EDGE);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE);bitmap.close();return texture;}
