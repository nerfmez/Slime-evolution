...
export function reflectEliteTurtleProjectile(world,e,player,damage=8){
  if(!e?.elite||!turtleShieldActive(e)||!world||!player)return false;
  const dx=player[0]-e.x,dz=player[2]-e.z,d=Math.max(.001,Math.hypot(dx,dz));
  (world.bullets??=[]).push({kind:'turtle-reflect',owner:e.id,x:e.x,z:e.z,y:.34,vx:dx/d*5.6,vz:dz/d*5.6,life:2.2,age:0,radius:.12,damage:Math.max(2,Math.round(damage*.55))});
  e.turtleShieldFlash=.26;
  (world.hazards??=[]).push({kind:'turtle-reflect-flash',x:e.x,z:e.z,r:.72,age:0,life:.28,damage:0});
  (world.eliteFx??=[]).push({kind:'turtle-reflect',x:e.x,z:e.z,r:.82,age:0,life:.30});
  return true;
}
...
export async function createPondTurtleRenderer(gl,{program,geometry,uniform,render}){
  ...
  const p=program(gl,`layout(location=0)in vec3 position;layout(location=2)in vec2 uv;
    ...
    uniform float flipX,alpha,emissive,elite;out vec4 color;
    ...
    if(elite>.5){float shell=smoothstep(.04,.20,c.g-c.b)*smoothstep(.88,.42,max(c.r,max(c.g,c.b)));vec3 accent=vec3(c.r*.78,min(1.,c.g*.92+.035),min(1.,c.b*1.12+.035));c.rgb=mix(c.rgb,accent,.40*shell);}
  ...
  return {stats,draw(e,vp,player,camera=[0,12,15]){
    ...
    const values={vp,origin:[e.x,.025,e.z],player,size:baseSize,anchor:TURTLE_FOOT,cameraDirection:camera,flipX:facing<0?1:0,alpha:pose.alpha,emissive:0,elite:e.elite?1:0,
