...
export async function createWaterCalfRenderer(gl,{program,geometry,uniform,render}){
  ...
  `in vec2 UV;in vec3 world;uniform sampler2D atlas,canopy;uniform vec4 rect;uniform vec3 player;uniform float flipX,alpha,lit,elite;out vec4 color;
  ...
  if(elite>.5){float waterTone=smoothstep(.02,.16,c.b-c.r)*smoothstep(.32,.62,c.b);vec3 accent=vec3(c.r*.88,min(1.,c.g*1.04+.025),min(1.,c.b*1.10+.025));c.rgb=mix(c.rgb,accent,.34*waterTone);}
  ...
  function drawCell(cell,origin,vp,player,size,flip,alpha,angle,anchor,lit,camera,elite=0){
    ...
    gl.useProgram(p);
    for(const [key,value] of Object.entries({vp,origin,player,size,flipX:flip,alpha,angle,anchor,lit,elite,cameraDirection:camera,rect:[cell%4*.25,1-(Math.floor(cell/4)+1)*.2,.25,.2]}))uniform(gl,p,key,value);
  ...
  return {stats,draw(e,vp,player,camera=[0,12,15]){
    ...
    return drawCell(pose.cell,[e.x,.025,e.z],vp,player,WATER_CELL_WORLD*(e.scale||1),waterFacing(e)<0?1:0,pose.alpha,0,WATER_FOOT,1,camera,e.elite?1:0);
