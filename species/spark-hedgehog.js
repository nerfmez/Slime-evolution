...
export function tickSparkWorld(world,dt){
  ...
  for(const e of world.enemies)if(e.type==='spark')e.sparkEliteBurst=Math.max(0,(e.sparkEliteBurst||0)-dt);
}
...
export function sparkMelee(world,e,dt,target,distance,canSee,damage){
  ...
  if(before<SPARK_IMPACT_TIME&&after>=SPARK_IMPACT_TIME&&!e.sparkHitApplied){
    ...
    if(e.elite){
      const radius=1.65;
      (world.hazards??=[]).push({kind:'elite-lightning',x:e.x,z:e.z,r:radius,age:0,life:.46,damage:0});
      (world.eliteFx??=[]).push({kind:'spark-ring',x:e.x,z:e.z,r:radius,age:0,life:.46});
      if(Math.hypot(target[0]-e.x,target[2]-e.z)<radius+(world.playerRadius||.25))world.damage(Math.max(1,Math.round(damage*.72)));
      e.sparkEliteBurst=.46;
    }
  }
...
export async function createSparkHedgehogRenderer(gl,{program,geometry,uniform,render}){
  ...
  const p=program(gl,`layout(location=0)in vec3 position;layout(location=2)in vec2 uv;
    ...
    `in vec2 UV;in vec3 world;uniform sampler2D atlas,canopy;uniform vec4 rect;uniform vec3 player;uniform float flipX,alpha,elite;out vec4 color;
    ...
    if(elite>.5){float hi=max(c.r,max(c.g,c.b)),lo=min(c.r,min(c.g,c.b));float sat=hi-lo;float spine=smoothstep(.10,.28,sat)*smoothstep(.82,.38,c.g);vec3 accent=vec3(c.r*.72,c.g*.78,min(1.,c.b*1.28+.10));c.rgb=mix(c.rgb,accent,.52*spine);}
  ...
  return {stats,draw(e,vp,player,camera=[0,12,15]){
    ...
    const values={vp,origin:[e.x,.025,e.z],player,size:SPARK_CELL_WORLD*(e.scale||1),anchor:SPARK_FOOT,
      cameraDirection:camera,flipX:facing<0?1:0,alpha:pose.alpha,elite:e.elite?1:0,rect:[cell%4*.25,1-(Math.floor(cell/4)+1)*.25,.25,.25]};
