...
function mount(){
  ...
  r.innerHTML=`
    ...
    <div id="sm-preview" style="border:1px solid #c9c5ab;border-radius:14px;padding:10px;margin:8px 0 12px;background:#eef0d9;overflow:hidden">
      ...
      <label style="display:grid;grid-template-columns:1fr auto;gap:4px;margin:8px 0 0">ตำแหน่งในรอบ <output id="sm-preview-phase">กำลังเล่น</output><input id="sm-preview-scrub" type="range" min="0" max="1" step="0.01" value="0" disabled style="grid-column:1/-1;width:100%"></label>
  ...
  const cells=[[1,2],[2,2],[2,1],[2,0],[1,0],[0,0],[0,1],[0,2]];
  const ordered=[4,3,2,1,0,7,6,5];
  ...
  for(const i of ordered){const b=document.createElement('button');b.type='button';b.dataset.dir=i;b.textContent=dirLabel(i);b.style.cssText='min-height:40px;padding:5px 4px';b.onclick=()=>{selected=i;markDir();drawPreview(performance.now());};dirBox.append(b)}markDir();

  function fitVisible(img,row,col){
    const cw=img.naturalWidth/3,ch=img.naturalHeight/3,bb=A.runtime?.b?.[row]?.[col]||[0,0,1,1];
    const sx=(col+bb[0])*cw,sy=(row+bb[1])*ch,sw=Math.max(1,(bb[2]-bb[0])*cw),sh=Math.max(1,(bb[3]-bb[1])*ch);
    const ratio=sw/sh,baseH=Math.min(235,172*(A.tune.scale||1)),baseW=baseH*ratio;
    return{sx,sy,sw,sh,baseW,baseH};
  }
  function drawArrow(x,y,ang,len){
    ctx.save();ctx.translate(x,y);ctx.rotate(ang);ctx.strokeStyle='#50624e';ctx.fillStyle='#50624e';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(-len*.5,0);ctx.lineTo(len*.5,0);ctx.stroke();ctx.beginPath();ctx.moveTo(len*.5,0);ctx.lineTo(len*.5-14,-9);ctx.lineTo(len*.5-14,9);ctx.closePath();ctx.fill();ctx.restore();
  }
  function drawPreview(now){
    const W=canvas.width,H=canvas.height,T=A.tune,img=A.runtime?.img;
    ...
    const [col,row]=cells[selected],q=fitVisible(img,row,col),cy=226,cx=W/2,Tcycle=Math.max(.05,T.cycle||.96);
    const base=playing?now/1000/Tcycle:pausedPhase;
    const phase=base*Math.PI*2+(T.phase||0)*Math.PI/180,g=(T.g?.[selected]??1),wave=T.on?Math.sin(phase)*g:0;
    const along=T.on?1+(T.stretch||0)*wave:1,cross=T.on?1-(T.squash||0)*wave:1;
    const ang=Math.PI/2-selected*Math.PI/4+(T.axis||0)*Math.PI/180;
    const lead=T.on?(along-1)*(q.baseH/2)*(T.lead||0):0,ldx=Math.cos(ang)*lead,ldy=Math.sin(ang)*lead;
    ...
    ctx.save();ctx.translate(cx+ldx,cy+ldy);ctx.rotate(ang);ctx.scale(along,cross);ctx.rotate(-ang);ctx.drawImage(img,q.sx,q.sy,q.sw,q.sh,-q.baseW/2,-q.baseH/2,q.baseW,q.baseH);ctx.restore();
    drawArrow(cx,382,ang,92);
  ...
  function add(d,to=c){
    const[k,name,t,a,b,s]=d,l=document.createElement('label');l.style.cssText='display:grid;grid-template-columns:1fr auto;gap:4px;margin:8px 0';l.append(document.createTextNode(name));
    if(t==='b'){const i=document.createElement('input');i.type='checkbox';i.checked=!!A.tune[k];i.onchange=()=>{A.tune[k]=i.checked;show();drawPreview(performance.now())};l.append(i);M.set(k,i)}
    else{const o=document.createElement('output'),i=document.createElement('input');i.type='range';i.min=a;i.max=b;i.step=s;i.value=A.tune[k];const f=()=>o.textContent=(+i.value).toFixed(s<.01?3:s<1?2:0);f();i.style.gridColumn='1/-1';i.oninput=()=>{A.tune[k]=+i.value;f();show();drawPreview(performance.now())};l.append(o,i);M.set(k,{i,o,s})}
    to.append(l);
  }
  ...
  const timer=setInterval(()=>r.open&&show(),220);
  r.addEventListener('toggle',()=>{if(r.open)drawPreview(performance.now())});
  window.addEventListener('pagehide',()=>{cancelAnimationFrame(raf);clearInterval(timer)},{once:true});
  show();drawPreview(performance.now());
}
