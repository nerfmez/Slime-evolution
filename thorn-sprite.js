...
const HIT_CELL=9; // Approved: death frame 2 is also the hit frame.
...
function frogFrame(e){
 ...
 if(e.hit>0)return {rect:cellRect(HIT_CELL),width:1,offset:0};
 if((e.__frogAttackPose||0)>0)return {rect:ATTACK_RECT,width:3,offset:1.64};
...
function patchFrogVisualState(){
 ...
 proto.__mossFrogVisualPatch=true;
...
function loadImage(url){
 return new Promise((resolve,reject)=>{
  ...
  image.onerror=()=>reject(Error('โหลดภาพ Moss Frog ไม่สำเร็จ: '+url));
  image.src=url;
 });
}
